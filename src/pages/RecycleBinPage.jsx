...
export default function RecycleBinPage() {
  ...
  const handleRestore = async (item) => {
    ...
    if (item.payload) {
      ...
      if (p.id) await unmarkDeletedId(p.id).catch(console.warn);
      if (p.invoice_number) await unmarkDeletedId(p.invoice_number).catch(console.warn);
      if (p.vehicle_number) await unmarkDeletedId(p.vehicle_number).catch(console.warn);

      if (item.item_type === 'Inventory') {
        const currentInv = JSON.parse(localStorage.getItem('inventory_items') || '[]');
        localStorage.setItem('inventory_items', JSON.stringify([p, ...currentInv]));
        pushCloudInventoryItem(p).catch(console.warn);
      } else if (item.item_type === 'Billing Invoice') {
        const currentInvs = JSON.parse(localStorage.getItem('local_invoices') || '[]');
        localStorage.setItem('local_invoices', JSON.stringify([p, ...currentInvs]));
        pushCloudInvoice(p).catch(console.warn);

        // Auto-restore to KhataBook if payment is pending / unpaid
        const pendingAmt = parseFloat(p.due_amount || (p.payment_status !== 'PAID' ? p.grand_total || p.total_amount : 0));
        if (pendingAmt > 0) {
          const khataItem = {
            id: `khata_${p.id || Date.now()}`,
            customer_name: p.customer_name || 'Valued Customer',
            vehicle_number: p.vehicle_number || '',
            mobile_number: p.mobile_number || p.phone || '',
            amount_due: pendingAmt,
          ...
          localStorage.setItem('khata_entries', JSON.stringify([khataItem, ...currentKhata]));
          pushCloudKhataEntry(khataItem).catch(console.warn);
        }

        // Auto-restore to Customer list
        if (p.customer_name || p.mobile_number) {
          const custItem = {
            id: `cust_${p.mobile_number || p.customer_name}`,
            name: p.customer_name || 'Customer',
            mobile_number: p.mobile_number || p.phone || '',
            vehicle_number: p.vehicle_number || '',
            total_visits: 1,
            last_visit: p.date || new Date().toISOString().split('T')[0]
          };
          const currentCusts = JSON.parse(localStorage.getItem('local_customers') || '[]');
          localStorage.setItem('local_customers', JSON.stringify([custItem, ...currentCusts]));
          pushCloudCustomer(custItem).catch(console.warn);
        }

      } else if (item.item_type === 'Workshop Job') {
        const currentJobs = JSON.parse(localStorage.getItem('workshop_jobs') || '[]');
        localStorage.setItem('workshop_jobs', JSON.stringify([p, ...currentJobs]));
        pushCloudJob(p).catch(console.warn);

        // Auto-restore to KhataBook if payment pending
        const pendingAmt = parseFloat(p.due_amount || (p.status !== 'FINISHED' && p.payment_status !== 'PAID' ? p.live_total || p.total_amount : 0));
        if (pendingAmt > 0) {
          const khataItem = {
            id: `khata_job_${p.id}`,
            customer_name: p.customer_name || 'Valued Customer',
        ...
        const currentKhata = JSON.parse(localStorage.getItem('khata_entries') || '[]');
        localStorage.setItem('khata_entries', JSON.stringify([p, ...currentKhata]));
        pushCloudKhataEntry(p).catch(console.warn);
      } else if (item.item_type === 'Customer Record') {
        const currentCusts = JSON.parse(localStorage.getItem('local_customers') || '[]');
        localStorage.setItem('local_customers', JSON.stringify([p, ...currentCusts]));
        pushCloudCustomer(p).catch(console.warn);
      } else if (item.item_type === 'Online Booking') {
        const currentBookings = JSON.parse(localStorage.getItem('local_bookings') || '[]');
        localStorage.setItem('local_bookings', JSON.stringify([p, ...currentBookings]));
        pushCloudBooking(p).catch(console.warn);
      } else if (item.item_type === 'Contact Inquiry / Message') {
        const currentMsgs = JSON.parse(localStorage.getItem('local_messages') || '[]');
        localStorage.setItem('local_messages', JSON.stringify([p, ...currentMsgs]));
        pushCloudMessage(p).catch(console.warn);
      }
    }
